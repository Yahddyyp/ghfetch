...
impl Colors {
    ...
    pub fn from_config(config: &Config) -> Self {
        ...
        Colors {
            ...
            followers: c.followers.map(Color::to_rgb).unwrap_or((250, 179, 135)),
            repos: c.repos.map(Color::to_rgb).unwrap_or((116, 199, 236)),
            joined: c.joined.map(Color::to_rgb).unwrap_or((137, 220, 235)),
            company: c.company.map(Color::to_rgb).unwrap_or((250, 179, 135)),
